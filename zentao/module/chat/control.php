...
class chat extends control
{
    ...
    public function settings($account = '', $settings = '', $userID = 0)
    {
        ...
        if($settings)
        {
            $this->setting->setItem("system.chat.settings.$account", helper::jsonEncode($settings));
        }
        ...
        else
        {
            $this->output->result = 'success';
            $this->output->users  = array($userID);
            $this->output->data   = !empty($settings) ? $settings : json_decode($this->setting->getItem("owner=system&module=chat&section=settings&key=$account"));
        }
