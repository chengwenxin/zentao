...
$lang->cn->links['zentaobiz']['link']   = '/biz/';
$lang->cn->links['zentaobiz']['text']   = '企业版';
$lang->cn->links['zentaobiz']['target'] = '_self';
...
$lang->en->links['zentaobiz']['link']   = '/biz/';
$lang->en->links['zentaobiz']['text']   = 'Biz Version';
$lang->en->links['zentaobiz']['target'] = '_self';
    ...
    <div class='modal-content'>
      <div class='modal-body'>
        <div class="row">
          ...
          <div class="col-xs-8" style="border-left: 1px solid #ddd; padding: 25px 0 35px 8px">
...
$(function()
{
	$.getJSON('/pro/api-getLicenses.html', function(data)
	{
        ...
        if(data.company == 'try') $('#zentaopro').append('<?php echo $acceptLang == 'en' ? ' try' : ' 试用'?>');
        <?php
        $allLife    = $acceptLang == 'en' ? 'All Life' : '永久授权';
        $expireDate = $acceptLang == 'en' ? 'Expire Date : ' : '到期时间：';
        $unlimited  = $acceptLang == 'en' ? 'Unlimited' : '不限人数';
        $limitUser  = $acceptLang == 'en' ? 'Authorized : ' : '授权人数：';
        ?>
	...
	{
		if(!data) return false;
        if(data.company == 'try') $('#zentaobiz').append('<?php echo $acceptLang == 'en' ? ' try' : ' 试用'?>');
        <?php
        $allLife    = $acceptLang == 'en' ? 'All Life' : '永久授权';
        $expireDate = $acceptLang == 'en' ? 'Expire Date : ' : '到期时间：';
        $unlimited  = $acceptLang == 'en' ? 'Unlimited' : '不限人数';
