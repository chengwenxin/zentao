...
class chatModel extends model
{
    ...
    {
        $user = $this->dao->select('id, account, realname, avatar, role, dept, status, gender, email, mobile, phone,  qq, deleted')->from(TABLE_USER)->where('id')->eq($userID)->fetch();
    ...
    {
        $dao = $this->dao->select('id, account, realname, avatar, role, dept, status, gender, email, mobile, phone,  qq, deleted')
            ->from(TABLE_USER)
            ->where(1)
