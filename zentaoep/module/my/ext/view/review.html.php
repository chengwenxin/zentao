...
<div id='featurebar'>
  <ul class='nav'>
    <?php foreach($lang->my->reviewTypeList as $reviewType => $reviewName):?>
    <?php $class = $type == $reviewType? "class='active'" : ''?>
    <?php echo "<li $class>" . html::a(inlink('review', "type=$reviewType&orderBy=$orderBy&from=$from"), $reviewName) . '</li>';?>
    <?php endforeach;?>
  </ul>
</div>
...
<div class='panel'>
  <table class='table table-data table-hover text-center table-fixed tablesorter <?php echo $appendClass?>' id='leaveTable'>
    <thead>
      <tr class='text-center'>
        <?php $vars = "type={$type}&orderBy=%s&from=$from";?>
